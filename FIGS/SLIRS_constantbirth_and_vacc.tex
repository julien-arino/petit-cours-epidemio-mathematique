...
\begin{tikzpicture}[auto, %node distance = 2cm, auto,
    ...
    \node [cloud, right=of S, fill=red!30] (L) {$L$};
	...
	%% Arrows
	\path [line, very thick] (S) to node [midway, below] (TextNode) {$\beta SI$} (L);
	...
	%% Birth and death
	\path [line, very thick] (bS) to node [right] (TextNode) {$(1-p)b$} (S);
	\path [line, very thick] (bR) to node [left] (TextNode) {$pb$} (R);
